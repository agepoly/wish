...
SOCKET.on("get data", function (content, mailing_in_progress) {
  ...
  if (
    !mailing_in_progress &&
    content.participants.some(function (p) {
      return p.status <= 10;
    })
  ) {
    var mails_error = content.participants
      .filter(function (p) {
        return p.status < 0;
      })
      .map(function (p) {
    ...
    var mails_new = content.participants
      .filter(function (p) {
    ...
    var mails_update = content.participants
      .filter(function (p) {
        return p.status == 10;
      })
      .map(function (p) {
