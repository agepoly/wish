  ...
  function send_data(key, mailing_in_progress) {
    db.collection("events").findOne({ _id: key }, null, function (err, event) {
      ...
      db.collection("participants")
        .find({ event: key })
        .toArray(function (err, participants) {
  ...
  socket.on("set data", function (content, send_mails) {
    ...
    db.collection("events").findOne(
      ...
      function (err, event) {
        ...
        if (
          content.participants.length > vmax ||
          content.participants.length < vmin
        ) {
...
The Wish team</p>`,
                  ...
                  for (var j = 0; j < content.participants.length; ++j) {
                    ...
                    if (content.participants[j].status <= 0) {
                      ...
                      mailer.send(
                        ...
                        check_mail(
                          content.participants[j]._id,
                          content.participants[j].mail,
                        ),
                      );
                    } else if (content.participants[j].status <= 10) {
                      // 10 == update not sent
                      total_mails++;
                      mailer.send(
                        {
                ...
                function check_mail(id, mail) {
                  return function (err, message) {
                    if (err) {
                      errors += Mustache.render(
                        "<p>Error with <strong>{{mail}}</strong>: <i>{{error}}</i>",
                        {
                          mail: mail,
                          error: String(err),
                        },
                      ...
                      db.collection("participants").updateOne(
                        { _id: id },
                        { $set: { status: 20 } },
                      ); // 20 == mail sent but no activity
                    }
                    if (errors) {
                      socket.emit("feedback", {
                        title: "Mails status",
                        html: Mustache.render(
                          "<p>{{sent}} over {{total}} mails sent</p>{{{errors}}}",
                          {
                            sent: sent_mails,
                            total: total_mails,
                            errors: errors,
...
The Wish team</p>`,
    ...
    db.collection("events").findOne(
      ...
      function (err, event) {
        ...
        db.collection("participants")
          .find({ _id: { $in: event.participants }, status: { $lt: 35 } })
          .toArray(function (err, participants) {
