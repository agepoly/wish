...
fn shuffle(vmin: &Vec<u32>,
           vmax: &Vec<u32>,
           mut wishes: Vec<Vec<f64>>,
           rand: &mut rand::XorShiftRng)
           -> Vec<usize> {
    ...
    // Modify slowly the wishes according to the attractivity of each workshop
    // up to eveybody is in his "first choice" (modified first choice)
    loop {
...
pub fn search_solution(vmin: &Vec<u32>,
                       vmax: &Vec<u32>,
                       wishes: &Vec<Vec<u32>>,
                       time: f64)
    ...
    if user_total > vmax_total || user_total < vmin_total {
        return Err(format!("capacity problem : vmin {} <= total users {} <= vmax {}",
                           vmin_total,
                           user_total,
                           vmax_total));
    }
    ...
    loop {
        ...
        if time::precise_time_s() - t_print > 1.0 {
            ...
            println!("Iter {it:>5} ({rate:>4.0}/s). Actual best score : {bs} x {nbs}. \
                      {left:>4.1} seconds left ",
                     bs = best_score,
